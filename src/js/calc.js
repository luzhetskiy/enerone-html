...
function getPercentage(consumptionVolume) {
  const threshold = consumptionThresholds.find(
    (t) => consumptionVolume <= t.max
  );
...
const initCalcForm = (form, data) => {
  ...
  const inputConsumptionVolume = form.querySelector(
    'input[name="consumption-volume"]'
  );
  ...
  choiceRegion.addEventListener("change", () => {
    ...
    renderChoice(choiceSupplier, data, (items) =>
      items
  ...
  choiceSupplier.addEventListener("change", () => {
    ...
    renderChoice(choiceMaxPower, data, (items) =>
      items
        .filter(
          (i) =>
            i["region"] === selectedRegion && i["company"] === selectedSupplier
        )
        .flatMap((i) => i.rates)
    );
  ...
  inputConsumptionVolume.addEventListener("input", () => {
    inputConsumptionVolume.value = formatNumber(
      inputConsumptionVolume.value.replace(/\D/g, "")
    );

    const value = parseFormattedNumber(inputConsumptionVolume.value);
    const valueChoiceMaxPower =
      parseFloat(getChoices(choiceMaxPower).getValue(true).replace(",", ".")) ||
      0;
    ...
    if (value && valueChoiceMaxPower && isValid) {
      ...
      resultDisplay.textContent = `${Math.trunc(result).toLocaleString(
        "ru-RU"
      )}\u00A0₽`;
...
fetch("../calc.json")
  .then((res) =>
    res.ok ? res.json() : Promise.reject(`Ошибка загрузки JSON: ${res.status}`)
  )
  .then((data) => {
